    ...
    try {
        ...
        if (!cart || cart.items.length === 0) {
            return res.render('../views/user/checkout', {
                user: userId, cartItems: [], cartTotal: 0, addresses, deleveryCharge: 0, greatestDiscounts: [], coupons: []
            });
        }
        ...
        const cartTotal = Math.floor(cartItemsWithDiscounts.reduce((total, item) => total + (item.itemPrice * item.quantity), 0));
        const deliveryCharge = cartTotal > 500 ? 0 : 40.00;
        ...
        res.render("../views/user/checkout", {
            ...
            greatestDiscounts: cartItemsWithDiscounts.map(item => item.greatestDiscount)
        });
    } catch (error) {
    ...
    try {
        ...
        for (const item of cart.items) {
            ...
            if (!product || product.stock < item.quantity) {
                return res.status(400).json({ success: false, message: `Insufficient stock for ${product.name}. Available stock: ${product.stock}` });
            }

            let greatestDiscount = 0;
            if (product.offer && product.offer.length > 0) {
                const activeOffers = product.offer.filter(offer => offer.isActive);
                if (activeOffers.length > 0) greatestDiscount = Math.max(...activeOffers.map(offer => offer.discount));
            }

            product.stock -= item.quantity;
            await product.save();

            item.discountedPrice = product.price - (product.price * (greatestDiscount / 100));
        }
        ...
        const order = new Order({
            ...
            items: cart.items.map(item => ({
                ...
                price: Math.floor(item.discountedPrice) || item.product.price
            })),
            status: paymentMethod === 'Razorpay' ? "Pending" : "Order placed",
            couponCode,
            couponDiscount: couponDiscount.toFixed(2),
            subtotal: cart.items.reduce((t, i) => t + (i.product.price * i.quantity), 0).toFixed(2),
            deleveryCharge,
